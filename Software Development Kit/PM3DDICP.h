...
#define PM3PROC_TYPES
   ...
   typedef void (ASYNCAPI *PM3EVENT_PROC)(unsigned short, unsigned char);
   typedef void (ASYNCAPI *MPDEVENT_PROC)(unsigned short, unsigned char, char *, char *);
   typedef void (ASYNCAPI *PM3ASYNC_PROC)(unsigned short, unsigned char *, unsigned short, short, short);
   typedef void (ASYNCAPI *PM3ASYNC_CSAFE_PROC)(unsigned short, unsigned long *, unsigned short, short, short);
#endif

typedef void (ASYNCAPI *PM3LOG_PACKET_PROC)(
											unsigned char,   // message type: command or response
											unsigned short,  // port number
											unsigned short,  // command id tag
											unsigned char *, // pointer to data buffer
											unsigned short   // buffer size
											);
typedef void (ASYNCAPI *PM3LOG_MESSAGE_PROC)(
											 unsigned int,	// error code
											 const char * 	// pointer to message string
											 );
...
/* Definitions for packet log message types */
#define CMD_MSG		0
#define RSP_MSG		1
#define TAG_MSG		3
...
/****************************************************************************
   tkcmdsetDDI_init_log

   About:      Initializes the packet logging mechanism.

   Inputs:     PM3LOG_MESSAGE_PROC		function for logging a message
   	   	   	   PM3LOG_PACKET_PROC		function for logging packet data
...
****************************************************************************/
PM3DDI_API ERRCODE_T tkcmdsetDDI_init_log(PM3LOG_MESSAGE_PROC, PM3LOG_PACKET_PROC);
...
/****************************************************************************
 tkcmdsetDDI_write_spiflashblk8
 
 About:     Writes an 8-bit block of data to a SPI FLASH memory location
            Note: The USB implementation supports only 1 block write at at time
            (depends on TKUSB_MAX_PACKET_SIZE)
 ...
 ****************************************************************************/
PM3DDI_API ERRCODE_T tkcmdsetDDI_write_spiflashblk8(UINT16_T port, UINT32_T address, UINT8_T * val_ptr8,
                                                    UINT32_T len, UINT16_T * packet_rsp_len);


/****************************************************************************
 tkcmdsetDDI_read_spiflashblk8
 
 About:     Reads an 8-bit block of data from a FLASH memory location
            Note: The USB implementation supports only 1 block read at at time
            (depends on TKUSB_MAX_PACKET_SIZE)
 ...
 ****************************************************************************/
PM3DDI_API ERRCODE_T tkcmdsetDDI_read_spiflashblk8(UINT16_T port, UINT32_T address, UINT8_T * val_ptr8,
                                                   UINT32_T len, UINT16_T * packet_rsp_len);

/****************************************************************************
 tkcmdsetDDI_get_image_crc
 
 About:     Returns 32-bit CRC of a firmware image
 
 Inputs:    UINT8_T     port            Identifier for device
            UINT32_T    address         Starting address of image
            UINT32_T    len             Number of bytes to perform CRC
            UINT16_T *  packet_rsp_len  Expected response packet size
            (used to allow lower levels to select the correct response report ID)
 
 Outputs:   UINT8_T *   val_ptr         Location to store calculated CRC
 ...
 ****************************************************************************/
PM3DDI_API ERRCODE_T tkcmdsetDDI_get_image_crc(UINT16_T port, UINT32_T address, UINT32_T *val_ptr32,
                                               UINT32_T len, UINT16_T *packet_rsp_len);

/****************************************************************************
 tkcmdsetDDI_get_update_status
 
 About:     Returns firmware update status
 
 Inputs:    UINT8_T     port            Identifier for device
            UINT16_T *  packet_rsp_len  Expected response packet size
            (used to allow lower levels to select the correct response report ID)
 
 Outputs:   UINT16_T *   val_ptr         Location to store update type and status
 
 Returns:   ERRCODE_T   ecode           Zero if successful
                                        Error code otherwise
 
 ****************************************************************************/
PM3DDI_API ERRCODE_T tkcmdsetDDI_get_update_status(UINT16_T port, UINT16_T *val_ptr16,
                                                UINT16_T *packet_rsp_len);
...
/****************************************************************************
 tkcmdsetDDI_get_buffer_mode
 
 About:      Returns the USB buffer mode
 
 Inputs:     UINT16_T    port        Communication port to use
 UINT8_T *mode
 
 Outputs:    None - updates mode
 
 Returns:    ERRCODE_T   ecode       Zero if successful
 Error code otherwise
 
 ****************************************************************************/
PM3DDI_API ERRCODE_T tkcmdsetDDI_get_buffer_mode(UINT16_T port, UINT8_T *mode);
...
/****************************************************************************
   tkcmdsetDDI_register_events2

   About:      Register a callback for device event functions. This utilizes
               a callback that has additional connection parameters 
               (product name and serial number).

   Inputs:     MPDEVENT_PROC	callback	Callback function to receive events
...
****************************************************************************/
PM3DDI_API ERRCODE_T tkcmdsetDDI_register_events2(MPDEVENT_PROC callback);

/****************************************************************************
   tkcmdsetDDI_unregister_events2

   About:      Unregister the device event2 callback
...
****************************************************************************/
PM3DDI_API ERRCODE_T tkcmdsetDDI_unregister_events2();
